...
import { ClusterRollupRow, queryCardRollupRows } from '../../shared/card-rollups/card-rollup.sql';
...
export class DashboardApiService {
  ...
  async getTrendProduct(id: string) {
    ...
    const clusterBase = await this.prisma.productCluster.findUnique({
      where: { id },
      select: { id: true, canonicalName: true, category: true, riskLevel: true, financialScore: true },
    });
    const snapshots = clusterBase
