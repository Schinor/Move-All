...
@Injectable()
export class DouyinConnector extends BaseHttpConnector {
  sourceName = 'douyin';
  sourceType: SourceType = 'social';
  protected readonly credentialConfigKey = 'sources.douyin.providerUrl';
  ...
  async getProductDetails(productId: string): Promise<RawProductDetail> {
    if (!this.isEnabled()) {
      throw new Error(`douyin: conector desabilitado`);
    }
