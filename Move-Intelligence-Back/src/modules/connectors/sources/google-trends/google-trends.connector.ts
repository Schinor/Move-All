...
@Injectable()
export class GoogleTrendsConnector extends BaseHttpConnector {
  sourceName = 'google-trends';
  sourceType: SourceType = 'search';
  protected readonly credentialConfigKey = 'sources.googleTrends.providerUrl';
  ...
  async getProductDetails(productId: string): Promise<RawProductDetail> {
    if (!this.isEnabled()) {
      throw new Error(`google-trends: conector desabilitado`);
    }
