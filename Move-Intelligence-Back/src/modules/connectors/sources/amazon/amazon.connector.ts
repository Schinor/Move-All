...
@Injectable()
export class AmazonConnector extends BaseHttpConnector {
  sourceName = 'amazon';
  sourceType: SourceType = 'third_party_scraper';
  protected readonly credentialConfigKey = 'sources.amazon.providerUrl';
  ...
  async getProductDetails(productId: string): Promise<RawProductDetail> {
    if (!this.isEnabled()) {
      throw new Error(`amazon: conector desabilitado`);
    }
